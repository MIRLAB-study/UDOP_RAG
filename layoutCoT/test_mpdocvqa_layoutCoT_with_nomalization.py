...
'''
        ...
        for retrieval_result in retrieval_results:
            ...
            normalized_answer = [a_answer.lower() for a_answer in answer]
            normalized_response = response.lower()
            answers.append({"retrieved_page":retrieved_page,"normalzied_answer" : normalized_answer,"normalzied_response":normalized_response, "anls":anls(normalized_answer, normalized_response), "retreive_score": 1 if retrieved_page==relevant_pages else 0})
            
            print(f"\nQuestion: {user_query}")
            print(f'retrieved page: {retrieved_page}')
            print(f"response: {normalized_response}")
            print(f"answer: {normalized_answer}")
            print(f"ANLS: {anls(normalized_answer, normalized_response)}")
            print(f"Relevant page: {relevant_pages}\n\n")
        json_data['data'][idx]['model_response'] = answers
        # temporal save the json file into the local path 
        with open('/home/yejinhand/project/UDOP_RAG/temp_qwenvl_result/val_with_response_qwenvl_layoutCOT_normalized.json' , 'w') as f:
            json.dump(json_data, f)
            
    with open(json_path.replace('.json', '_with_response_qwenvl_layoutCOT_normalized.json' ), 'w') as f:
        json.dump(json_data, f)
...
if __name__ == "__main__":
    ...
    args.add_argument('--end', type=str, help='end number of doc', default=5187)
